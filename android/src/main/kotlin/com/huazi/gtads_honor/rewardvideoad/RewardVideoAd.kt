package com.huazi.gtads_honor.rewardvideoad
...
import com.huazi.gtads_honor.FlutterHuaweiAdEventPlugin
...
object RewardVideoAd {
    ...
    private var rewardVideoAD: RewardAdLoad? = null
    ...
    private fun loadRewardVideoAd() {
        val builder = AdSlot.Builder().setSlotId(codeId)

// 此处示例通过广告SDK向广告服务器传递自定义数据
        builder.setAdContext(customData)

        val listener: RewardAdLoadListener = object : RewardAdLoadListener {
            override fun onLoadSuccess(p0: RewardExpressAd?) {
                // 激励广告加载成功
        ...
        // 激励视频的加载，仅做示例
        rewardVideoAD = RewardAdLoad.Builder().setRewardAdLoadListener(listener)
